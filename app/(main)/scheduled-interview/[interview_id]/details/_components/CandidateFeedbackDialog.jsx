              ...
              <DialogHeader>
                ...
                <DialogDescription asChild>
                    <div className=' mt-5'>
                        ...
                        <div className={` mt-10 p-5 flex justify-between items-center rounded-md ${feedback?.Recommendation=='No' ? 'bg-red-100' : 'bg-green-100'}`}>
                            <div>
                                <h2 className={` font-bold ${feedback?.Recommendation=='No' ? 'text-red-700' : 'text-green-700'}`}>Recommendation Msg</h2>
                                <p className={` mt-2 font-bold ${feedback?.Recommendation=='No' ? 'text-red-500' : 'text-green-500'}`}>{feedback?.RecommendationMsg}</p>
                            </div>
                            <Button className={` cursor-pointer ${feedback?.Recommendation=='No' ? 'bg-red-700 hover:bg-red-500' : 'bg-green-700 hover:bg-green-500 '}`} onClick={sendMsg}>Send Msg</Button>
                        </div>
                    </div>
                </DialogDescription>
              </DialogHeader>
